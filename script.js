...
/* ---------- Scroll parallax (hero depth + section lag) ---------- */
if (!reduceMotion) {
...
if (radarHost) {
  ...
  const labels = radarData.map((d, i) => {
    ...
    // label sits well clear of the vertex, and always extends AWAY from center
    // (never centered on the axis line, which is what caused the overlap)
