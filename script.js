...
if (langStat) {
  const closeLangPop = () => {
    langStat.classList.remove('is-open');
    langStat.setAttribute('aria-expanded', 'false');
  };
  const toggleLangPop = (e) => {
    e.stopPropagation();
    const isOpen = langStat.classList.toggle('is-open');
    langStat.setAttribute('aria-expanded', String(isOpen));
  };
  langStat.addEventListener('click', toggleLangPop);
  langStat.addEventListener('keydown', (e) => {
    if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleLangPop(e); }
    if (e.key === 'Escape') closeLangPop();
  });
  document.addEventListener('click', (e) => {
    if (!langStat.contains(e.target)) closeLangPop();
  });
}
